    ...
    Friend Sub Control_New(Optional ByVal bClear As Boolean = False, Optional ByVal bOrderModify As Boolean = False)
        ...
        If Not class_Procedures.isEmpty(Brand) Then
            If bOrderModify Then
                ...
                Order_Code.EditValue = class_Database.Get_Last_ID(class_Database.ICSConnection, "Spare_Parts_Purchase_Order", "ID", "Order_Code", $"{sOrderCode(0)}-{sOrderCode(1)}-")
            Else
                Order_Code.EditValue = class_Database.Get_Last_ID(class_Database.ICSConnection, "Spare_Parts_Purchase_Order", "ID", "Order_Code", $"{class_Saga_Variables.sBranchCode}{Brand.Text.Trim}-", 1)
            End If
            Order_Date.Select()
        Else
            Brand.Select()
