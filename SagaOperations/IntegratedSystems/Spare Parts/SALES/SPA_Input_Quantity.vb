...
Public Class SPA_InputQuantity
    Sub Find_Spare_Parts()
        ...
        Using myDataReader As SqlDataReader = class_Database.Procedure_Retrieve(class_Database.SPAConnection, Parameters, "SPA_Find_PartsNumber")
    ...
    Sub Find_Spare_Parts_Master()
        ...
        Using myDataReader As SqlDataReader = class_Database.Procedure_Retrieve(class_Database.SPAConnection, Parameters, "SPA_Find_PartsNumber_Master")
            If myDataReader IsNot Nothing AndAlso myDataReader.HasRows Then
                myDataReader.Read()
                txt_PartsNumber.Text = myDataReader("PartsNumber").ToString()
                txt_PartsName.Text = myDataReader("PartsName").ToString()
    ...
    Sub Total()
        If txt_Quantity.Text = String.Empty Then
            txt_Quantity.Text = "0"
        End If
        txt_SubTotal.Text = Val(txt_TotalAmount.Text) * Val(txt_Quantity1.Text)
        txt_TotalLess.Text = Val(txt_LessAmount.Text) * Val(txt_Quantity1.Text)
        txt_GrossTotal.Text = Val(txt_Price.Text) * Val(txt_Quantity1.Text)
    End Sub
    ...
    Private Sub txt_Discount_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_Discount.KeyDown
        If e.KeyCode = Keys.Enter Then
            txt_TotalPercent.Text = Val(txt_Discount.Text) / Val(txt_Percent.Text)
            txt_LessAmount.Text = Val(txt_TotalPercent.Text) * Val(txt_Price.Text)
            txt_TotalAmount.Text = Val(txt_Price.Text) - Val(txt_LessAmount.Text)
    ...
    Private Sub txt_LessAmount_TextChanged(sender As Object, e As EventArgs) Handles txt_LessAmount.TextChanged
        txt_TotalAmount.Text = CInt(Val(txt_Price.Text) - Val(txt_LessAmount.Text))
        Total()
    End Sub
