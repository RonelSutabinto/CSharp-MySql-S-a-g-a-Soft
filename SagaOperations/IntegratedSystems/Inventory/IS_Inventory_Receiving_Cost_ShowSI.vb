...
Public Class IS_Inventory_Receiving_Cost_ShowSI
    ...
    Sub mg_ShowApprovedSIQuickSearch()
        LViewList.Items.Clear()
        Dim Parameters As SqlParameter() = {New SqlParameter("@ParamQuickSearch", QuickSearch.Text)}

        Using myDataReader As SqlDataReader = class_Database.Procedure_Retrieve(class_Database.ICSConnection, Parameters, "mg_ShowSIQuickSearch")
    ...
    Sub mg_ShowApprovedSI()
        LViewList.Items.Clear()
        Dim Parameters As SqlParameter() = {}

        Using myDataReader As SqlDataReader = class_Database.Procedure_Retrieve(class_Database.ICSConnection, Parameters, "mg_ShowSI")
            If myDataReader IsNot Nothing AndAlso myDataReader.HasRows Then
                Do While myDataReader.Read()
                    Dim li As ListViewItem = LViewList.Items.Add(myDataReader.Item("refno"))
                    li.SubItems.Add(myDataReader.Item("pono").ToString())
