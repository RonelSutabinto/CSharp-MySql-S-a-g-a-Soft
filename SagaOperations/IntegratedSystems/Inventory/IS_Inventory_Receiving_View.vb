...
Public Class IS_Inventory_Receiving_View
    ...
    Sub Approveditems()
        If class_Database.Data_Update(class_Database.ICSConnection, "UPDATE item_Master set inventoryStatus='Approved',branchCode = '" & MainMenu.statusBrCode.Caption & "',OriginatingMaker='" & txt_sender.Text & "',OriginatingBranch='" & txt_originatingBranch.Text & "',transactedby='" & MainMenu.App_User_Name.Caption & "',remarks='APPROVED' WHERE refno LIKE '" & txt_refno.Text & "' AND pono LIKE '" & txt_pono.Text & "' AND inventoryStatus LIKE '" & txt_invenStat.Text & "'") AndAlso
        class_Database.Data_Update(class_Database.ICSConnection, "UPDATE item_Details set IfExist='1',branchCode='" & MainMenu.statusBrCode.Caption & "',inventoryStatus='Approved' WHERE refno LIKE '" & txt_refno.Text & "' AND pono LIKE '" & txt_pono.Text & "'") Then
            MsgBox("The item/s has been approved!", vbInformation, "webSources")
        End If
    End Sub

    Sub ApproveditemsSameBranch()
        ...
        class_Database.Procedure_Save(class_Database.ICSConnection, Parameters, "mg_Inventory_ItemApproval", "Received Items Approved")
    End Sub
    ...
    Sub DisApproveditems()
        ...
        class_Database.Procedure_Save(class_Database.ICSConnection, Parameters, "mg_Inventory_ItemDisapproved", "Received Units Disapproved")
    End Sub
