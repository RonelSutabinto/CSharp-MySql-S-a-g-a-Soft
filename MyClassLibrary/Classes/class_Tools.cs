...
namespace MyClassLibrary.Classes
{
	public static class class_Tools
	{
		...
		public static void SubKeyTreeDelete(string regValue)
		{
			using (RegistryKey regkey = Registry.CurrentUser.OpenSubKey($@"{Application.ProductName}\\", true))
			{
				if (regkey.OpenSubKey(regValue) != null)
				{
					regkey.DeleteSubKeyTree(regValue);
				}
			}

			using (RegistryKey regkey = Registry.CurrentUser.OpenSubKey(@"\", true))
			{
				if (regkey.OpenSubKey(regValue) != null)
				{
					regkey.DeleteSubKeyTree(regValue);
